...
import { acceptProposal, createOrder, createProject, createProposal, getWorkspaceData, markAllNotificationsRead } from "@/services/api/workspace";
...
export default function DashboardPage() {
  ...
  async function submitProject(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await run(() => createProject({ title: data.get("title"), description: data.get("description"), garment_type: data.get("garment_type"), fabric: data.get("fabric"), quantity: Number(data.get("quantity")), budget: data.get("budget") || null, deadline: data.get("deadline") || null }), "درخواست شما ثبت شد و حالا برای خیاطان نمایش داده می‌شود.");
  ...
  return <main className="dashboard-page">
    <aside className="dashboard-sidebar">
      ...
      <nav className="dashboard-nav" aria-label="منوی اصلی">
        <a className="active" href="#overview">نمای کلی</a><a href="#projects">{isCustomer ? "درخواست‌های من" : "فرصت‌های خیاطی"}</a><a href="#orders">سفارش‌ها</a><a href="#notifications">اعلان‌ها {unread > 0 && <b>{unread.toLocaleString("fa-IR")}</b>}</a>
      </nav>
    ...
    <section className="dashboard-main">
      ...
      <div className="dashboard-columns"><section className="dashboard-section compact" id="orders"><div className="section-heading"><div><p>پیگیری کار</p><h2>سفارش‌های اخیر</h2></div></div>{orders.length ? orders.slice(0,4).map((order) => <article className="order-row" key={order.id}><div><strong>سفارش #{order.id.toLocaleString("fa-IR")}</strong><span>{isCustomer ? `خیاط: ${order.tailor_username}` : `مشتری: ${order.customer_username}`}</span></div><div><span className={`status status-${order.status}`}>{statusLabel[order.status]}</span><strong>{money(order.total_price)}</strong></div></article>) : <div className="empty-mini">هنوز سفارشی شکل نگرفته است.</div>}</section>
        <section className="dashboard-section compact" id="notifications"><div className="section-heading"><div><p>به‌روزرسانی‌ها</p><h2>اعلان‌ها</h2></div>{unread > 0 && <button onClick={() => run(markAllNotificationsRead, "همه‌ی اعلان‌ها خوانده شد.")}>خواندن همه</button>}</div>{notifications.length ? notifications.slice(0,5).map((item) => <article className={`notification-row ${item.is_read ? "" : "unread"}`} key={item.id}><i /><div><p>{item.message}</p><span>{date(item.created_at)}</span></div></article>) : <div className="empty-mini">اعلان تازه‌ای نداری.</div>}</section></div>
    </section>

    {projectModal && <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setProjectModal(false)}><div className="modal dashboard-modal"><button className="close-button" onClick={() => setProjectModal(false)}>×</button><p className="modal-kicker">درخواست دوخت جدید</p><h2>چه چیزی می‌خواهی بدوزی؟</h2><form onSubmit={submitProject}><label>عنوان درخواست<input name="title" required placeholder="مثلاً دوخت مانتوی تابستانی" /></label><label>توضیحات<textarea name="description" required placeholder="مدل، اندازه و جزئیات مهم را بنویسید" /></label><div className="form-pair"><label>نوع لباس<input name="garment_type" required placeholder="مانتو" /></label><label>نوع پارچه<input name="fabric" placeholder="لینن" /></label></div><div className="form-pair"><label>تعداد<input name="quantity" type="number" min="1" defaultValue="1" required /></label><label>بودجه (تومان)<input name="budget" type="number" min="0" /></label></div><label>مهلت تحویل<input name="deadline" type="date" /></label><button className="button button-primary" disabled={working}>{working ? "در حال ثبت…" : "ثبت و انتشار درخواست"}</button></form></div></div>}
